...
#('dim_reducer', LinearDiscriminantAnalysis(n_components=2), num_cols) ('encoder', OneHotEncoder(), cat_cols)
num_cols = dataframe.num_features()
cat_cols = dataframe.cat_features()
bin_cols = dataframe.binary_features()
transformer = ColumnTransformer(transformers=[('standard', StandardScaler(), num_cols), 
                                            ('encoder', OneHotEncoder(), cat_cols), 
                                            ('dim_reducer', LinearDiscriminantAnalysis(n_components=2), np.hstack((num_cols, bin_cols)))], 
                                            remainder='passthrough')          
models = [RandomForestClassifier(), GradientBoostingClassifier()]
params = [{'max_depth': range(10, 20),'n_estimators': range(3, 10)}, 
            {'n_estimators': range(75,100), 'max_depth': range(10,15)}]
colors = ['red', 'green']
...
for index, model in enumerate(models):
    mean_score, std_score, best_estimator = nested_cross_val(X_train, y_train, model, transformer, params[index])
    print(f'F_score - {mean_score} +/- {std_score} for Model_{index}')
    mean_tpr = 0
    mean_fpr = np.linspace (0, 1, 100)
    for i in range(0, len(np.unique(target))):
        fpr, tpr, roc_auc = plot_roc_curve(best_estimator, X_test, y_test, X_train, y_train, pos_1=i)
    ...
    ax.plot(mean_fpr, mean_tpr, color=colors[index], label=f'Model_{index}_mean_AUC - {mean_auc}')
ax.legend()
plt.show()